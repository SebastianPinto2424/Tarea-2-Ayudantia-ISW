...
export async function getPrivateProfile(req, res) {
  try {
    ...
    return handleSuccess(res, 200, "Perfil privado", {
      id: user.id,
      email: user.email,
...
export async function editPrivateProfile(req, res) {
  ...
  if (!meId) return handleErrorClient(res, 401, "No autenticado");
  ...
  try {
    const updated = await updateOwnProfile(meId, { email, password });
...
export async function deletePrivateProfile(req, res) {
  ...
  try {
    await deleteOwnAccount(meId);
    return handleSuccess(res, 200, "Cuenta eliminada correctamente", { id: meId });
  } catch (err) {
    const msg = String(err?.message || "");
